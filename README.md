...
- Product catalogue in the frontend.
- Public `GET` and `POST` API for products.
- Server-side validation: a product price cannot be negative.
...
| `BACKEND_API_URL` | frontend | URL Next.js uses to request the API. |
...
Public API fields are `id`, `name`, `description`, `price`, `image`, and `created_at`. `id` and `created_at` are generated by the server. The current MVP permits public product creation, so it should not be exposed as an unauthenticated production API.
...
- Only the product list and product creation endpoint exist; there are no product detail, update, or delete endpoints.
- Authentication, roles, cart, orders, payments, inventory, categories, filtering, pagination, and search are not implemented.
...
- Add authentication and restrict write operations.
- Add product detail and management endpoints with pagination and filtering.
- Add a cart and order flow.
